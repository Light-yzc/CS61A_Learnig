...
def balanced(m):
    ...
    if is_planet(m):
        return True
    w1 = length(left(m)) * total_mass(end(left(m)))
    w2 = length(right(m)) * total_mass(end(right(m)))
    return w1 == w2 and balanced(end(left((m)))) and balanced(end(right(m)))
# 得仔细研究
...
def max_path_sum(t):
    ...
    highest_num = 0
    for i in branches(t):
        highest_num = max(max_path_sum(i),highest_num)
    return highest_num + label(t)
