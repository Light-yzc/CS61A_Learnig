...
def buy(fruits_to_buy, prices, total_amount):
    ...
    def add(fruits, amount, cart):
        if fruits == [] and amount == 0:
            ...
            for k in range((price // prices[fruit]) + 1):
                # Hint: The display function will help you add fruit to the cart.
                add(fruits[1:], price - k * prices[fruit], cart + display(fruit,k + 1))
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    l1 =sqrt((get_lat(city_a) - lat) ** 2 + (get_lon(city_a) - lon) ** 2)
    l2 =sqrt((get_lat(city_b) - lat) ** 2 + (get_lon(city_b) - lon) ** 2)
    if l1 <= l2:
