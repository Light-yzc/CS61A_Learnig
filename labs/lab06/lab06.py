...
def make_change(amount, coins):
    ...
    if (amount -smallest) == 0:
        return [smallest]
    recurse = make_change(amount - smallest, rest)
    if recurse:
        return [smallest] + recurse
    else:
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        "*** YOUR CODE HERE ***"
        # smallist = min(self.coins)
        # if coin < smallist:
        #     return None
        # if coin == smallist:
        #     return [smallist]
        # rest = self.change(self, coin - smallist)
        # if (sum(rest) + smallist) == coin:
        #     self.coins[coin] = 1
        #     for i in rest.append(smallist):
        #         self.coins[i] -= 1
        # if rest:
        #     return rest + [smallist]
        # else:
        #     for i in self.coins:

        #         return self.change(self, coin - self.coins[i])
        ...
        if result:
            ...
            for key, value in self.coins.items():
                if value != 0:
                    list1.append(key)
                    list2.append(value)
            self.coins = dict(zip(list1,list2))
            return result
