...
def about(subject):
    ...
    def check(arg):
        for i in subject:
            if i in split(lower(remove_punctuation(arg))):
                for j in subject:
                    if i == j:
                        return True
        else:
...
def wpm(typed, elapsed):
    ...
    l = len(typed)
    words = l / 5
    words_per_sce = words / elapsed
    return words_per_sce * 60
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    return close_word if lowest_diff <= limit else typed_word
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    # BEGIN PROBLEM 6
#     diff = 0
#     l_gap =len(typed) - len(source)
#     if l_gap <= 0:
#         l1 = list(typed)
#         l2 = list(source)[:len(typed)]
#     else:
#         l1 = list(typed[:len(source)])
#         l2 = list(source)
#     for i,j in zip(l1,l2):
#         if i != j:
#             diff += 1
#     return diff + abs(l_gap)

# print(feline_fixes("goodbye", "good", 10))
    diff = 0
    l_gap = len(typed) - len(source)
    def corret(word1,word2,diff):
        if diff > limit:
            return limit + 1
        if len(word1) == 0:
            return diff - l_gap
        ...
        word1 = word1[1:]
        word2 = word2[1:]
        return corret(word1,word2,diff)
    return corret(typed,source,diff)
"""Use recursion"""
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)+1):
        if i == len(typed) or typed[i] != source[i]:
            progress = {'id': user_id, 'progress': num/to_len }
            upload(progress)
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    l_t = len(timestamps_per_player)
    l = len(timestamps_per_player[0])
    time = []
    for i in range(l_t):    
        time.append([])
        for j in range(l - 1):
            time[i].append(timestamps_per_player[i][j + 1] - timestamps_per_player[i][j]) 
...
def fastest_words(match):
    ...
    #         temp_word.append(times[j][i])
    #     index = temp_word.index(min(temp_word))
    #     words_1[index].append(words[i])
    # return words_1
    # format is worong
    # END PROBLEM 10
